...
import type { TopicSuggestion } from "../hooks/useQuestionTopics";
...
export function QuestionTopicDialog(props: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <Card className="mx-4 w-[min(96vw,1100px)] max-w-none overflow-hidden">
        ...
        <CardContent className="grid gap-5 p-5 sm:p-7 lg:grid-cols-[1fr_1fr] lg:p-8">
          <section className="min-h-[420px] space-y-4 rounded-2xl border bg-muted/20 p-5 shadow-sm">
            ...
            {props.suggestions.length ? <div className="space-y-2"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Chủ đề tương tự</p>{props.suggestions.map((topic) => <button key={topic.id} type="button" className="w-full rounded-xl border bg-background px-3 py-2.5 text-left hover:border-primary hover:bg-primary/5" onClick={() => props.onSelect(topic.id)}><span className="flex items-center justify-between gap-3"><span className="truncate text-sm font-medium">{topic.name}</span><span className="rounded-full bg-muted px-2 py-0.5 text-[10px]">{Math.round(topic.score * 100)}%</span></span>{topic.reason ? <span className="mt-1 block text-xs text-muted-foreground">{topic.reason}</span> : null}</button>)}</div> : null}
          </section>
          <section className="min-h-[420px] space-y-4 rounded-2xl border bg-background p-5 shadow-sm">
