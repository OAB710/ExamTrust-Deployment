...
const ai_profile_1 = require("./ai-profile");
let AiService = AiService_1 = class AiService {
    constructor(configService) {
        this.configService = configService;
        this.logger = new common_1.Logger(AiService_1.name);
        const apiKey = this.configService.get('GOOGLE_AI_API_KEY');
        this.provider = this.configService.get('AI_PROVIDER') || 'google';
        this.localUrl = this.configService.get('AI_LOCAL_URL') || undefined;
        this.ollamaUrl = this.configService.get('AI_OLLAMA_URL') || 'http://localhost:11434';
        this.ollamaModel = this.configService.get('AI_OLLAMA_MODEL') || 'gemma3:4b';
        this.nvidiaModel = this.configService.get('AI_NVIDIA_MODEL') || 'z-ai/glm-5.2';
        this.openRouterModel = this.configService.get('AI_OPENROUTER_MODEL') || 'nvidia/nemotron-3-ultra-550b-a55b:free';
        this.appName = this.configService.get('AI_APP_NAME') || 'Academic Trust Suite';
        this.defaultLanguage = this.configService.get('AI_DEFAULT_LANGUAGE') || 'vi';
        this.ollamaTemperature = Number(this.configService.get('AI_OLLAMA_TEMPERATURE') || 0.2);
        this.ollamaTopP = Number(this.configService.get('AI_OLLAMA_TOP_P') || 0.85);
        this.ollamaRepeatPenalty = Number(this.configService.get('AI_OLLAMA_REPEAT_PENALTY') || 1.1);
        this.ollamaNumCtx = Number(this.configService.get('AI_OLLAMA_NUM_CTX') || 8192);
        if (this.provider === 'google') {
            ...
            const nvidiaApiKey = this.configService.get('NVIDIA_API_KEY');
            const nvidiaBaseUrl = this.configService.get('AI_NVIDIA_BASE_URL') || 'https://integrate.api.nvidia.com/v1';
            if (!nvidiaApiKey) {
                this.logger.warn('NVIDIA_API_KEY not set. NVIDIA AI features will not work.');
            }
            this.nvidiaAI = new openai_1.default({
                apiKey: nvidiaApiKey || '',
            ...
            const openRouterBaseUrl = this.configService.get('AI_OPENROUTER_BASE_URL') || 'https://openrouter.ai/api/v1';
            const referer = this.configService.get('AI_OPENROUTER_HTTP_REFERER')
                || this.configService.get('APP_BASE_URL')
                || this.configService.get('FRONTEND_URL');
            const title = this.configService.get('AI_OPENROUTER_X_TITLE') || this.appName;
            if (!openRouterApiKey) {
                this.logger.warn('OPENROUTER_API_KEY not set. OpenRouter AI features will not work.');
            }
            this.openRouterAI = new openai_1.default({
    ...
    async generateQuestion(params) {
        const { prompt, questionType = 'MULTIPLE_CHOICE', difficulty = 0.5, language, courseName, useCase = 'question_bank', context, } = params;
        ...
        const profilePrompt = (0, ai_profile_1.buildExamTrustPromptHeader)({
...
- Return ONLY the JSON object, no additional text`;
        try {
            ...
            if (this.provider === 'ollama') {
                responseText = await this._callOllama(systemPrompt, this.buildOllamaOptions('question_generation'));
            }
            else if (this.provider === 'nvidia') {
    ...
    async generateExamQuestions(params) {
        const { prompt, questionCount, difficulty = 0.5, questionType, language, courseName, useCase = 'exam', context, } = params;
        const targetLanguage = language || this.defaultLanguage;
...
- Return ONLY the JSON object, no additional text`;
        try {
            let responseText;
            if (this.provider === 'ollama') {
                responseText = await this._callOllama(systemPrompt, this.buildOllamaOptions('exam_generation'));
            }
            else if (this.provider === 'nvidia') {
                responseText = await this._callNvidia(systemPrompt);
            }
...
- Return ONLY the JSON object, no additional text.`;
        try {
            let responseText;
            if (this.provider === 'ollama') {
                responseText = await this._callOllama(systemPrompt, this.buildOllamaOptions('grading_support'));
            }
            else if (this.provider === 'nvidia') {
                responseText = await this._callNvidia(systemPrompt);
            }
            ...
            const suggestions = parsed.suggestions
                .filter((s) => s && validQuestionIds.has(String(s.questionId)))
                .map((s) => ({
                ...
                .filter((s) => s.reasonSummary && s.recommendation);
            return {
...
- Return ONLY the JSON object, no additional text.`;
        try {
            ...
            const parsedSignals = parsed.signals
                .filter((s) => s && s.type && s.description)
                .map((s) => ({
    ...
    async suggestSimilarTopics(params) {
        ...
        const heuristicMatches = existingTopics
            ...
            else if (normalizedCandidate.includes(normalizedTopic) ||
...
- If nothing is similar, return an empty matches array.`;
        try {
            ...
            if (this.provider === 'ollama') {
                responseText = await this._callOllama(prompt, this.buildOllamaOptions('topic_matching'));
            }
            else if (this.provider === 'nvidia') {
    ...
    async _callOpenRouter(prompt) {
        const reasoningEnabled = String(this.configService.get('AI_OPENROUTER_REASONING_ENABLED') || '')
            .toLowerCase() === 'true';
        const request = {
    ...
    buildOllamaOptions(useCase) {
        return (0, ai_profile_1.getOllamaGenerationOptions)(useCase);
    }
    getTypeLabel(type) {
        const labels = {
