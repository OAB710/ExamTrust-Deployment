...
describe('ExamRiskAssessmentService', () => {
  ...
  const buildService = (overrides: { submission?: any; createJobResult?: any; existingJob?: any } = {}) => {
    ...
    const accessPolicy = {
      assertInstructorCanAccessExam: jest.fn().mockResolvedValue({ id: examId }),
      assertInstructorCanAccessSubmission: jest.fn().mockResolvedValue({ id: submissionId, examId }),
      assertInstructorCanAccessAnomalyFlag: jest.fn().mockResolvedValue({ id: 'flag-1' }),
    };

    const service = new ExamRiskAssessmentService(prisma as any, aiJobsService as any, accessPolicy as any);
    return { service, prisma, aiJobsService, accessPolicy };
  };
