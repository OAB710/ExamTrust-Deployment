...
const COURSE_NAME = 'Khóa học thử nghiệm Academic Trust';
...
async function main() {
  ...
  const exam1 = await ensureExam(course.id, lecturer.id, `${COURSE_CODE} - Bài thi thử không giới hạn lượt làm`, {
    duration: 45,
    timeLimitMinutes: 45,
    totalPoints: 20,
    passingScore: 10,
  ...
  const exam2 = await ensureExam(course.id, lecturer.id, `${COURSE_CODE} - Bài thi thử đã hoàn thành để phân tích`, {
    duration: 60,
    timeLimitMinutes: 60,
    totalPoints: 20,
    passingScore: 12,
